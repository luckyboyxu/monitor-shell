...
curl http://127.0.0.1:8081/server-status?auto > tmp/http.$$ 2> /dev/null
...
echo "{'server':'apache1','remark':'$remark','status':'$status','CpuLoad':'$CpuLoad','Uptime':'$Uptime','ReqPerSec':'$ReqPerSec','BytesPerSec':'$BytesPerSec','BytesPerReq':'$BytesPerReq','BusyWorkers':'$BusyWorkers','IdleWorkers':'$IdleWorkers','Scoreboard':'$scoreboard','date':'$mydate'}"
