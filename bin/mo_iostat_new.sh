...
do
	...
	result="${result},{'hostname':'$hostname','f_partition':'$partition','dirname':'$dirname','rrqm_s':'$rrqm_s','wrqm_s':'$wrqm_s','r_s':'$r_s','w_s':'$w_s','rsec_s':'$rsec_s','wsec_s':'$wsec_s','avgrq_sz':'$avgrq_sz','avgqu_sz':'$avgqu_sz','await':'$await','svctm':'$svctm','util':'$util','date':'$mydate','status':'$status','msg':'$message'}"
done < tmp/sed.$$
echo "[${result:1}]"
