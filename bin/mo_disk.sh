...
do
	...
	then
		status=3
		result="$result,{'hostname':'$hostname','status':3,'partition':'$partition','percent':$percent,'dirname':'$dirname','size':$size,'used':$used,'avail':$avail,'msg':'$hostname $dirname percent $percent%','date':'$mydate'}" 
	elif [ $percent -ge 90 ]
	then
		percent_pre=${tmp[3]}
		...
		if [ ${percent_pre} -ne -1 -a ${percent_incre} -ge 1 ];then
            ...
            result="$result,{'hostname':'$hostname','status':1,'partition':'$partition','percent':$percent,'dirname':'$dirname','size':$size,'used':$used,'avail':$avail,'msg':'$hostname $dirname percent $percent%,increase $percent_incre%','date':'$mydate'}" 
		else
			first_time=${tmp[2]}
			if [ ${first_time} -eq 0 ];then
				status=1
				result="$result,{'hostname':'$hostname','status':1,'partition':'$partition','percent':$percent,'dirname':'$dirname','size':$size,'used':$used,'avail':$avail,'msg':'$hostname $dirname percent $percent%','date':'$mydate'}"
			else
				...
				if [ ${status} -eq 1 ];then
					...
					result="$result,{'hostname':'$hostname','status':0,'partition':'$partition','percent':$percent,'dirname':'$dirname','size':$size,'used':$used,'avail':$avail,'msg':'nothing','date':'$mydate'}"
				fi
			fi
		fi
	else
